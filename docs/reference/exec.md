...
Display the code of an {rst:dir}`exec` block in an editor. If `ID` is provided,
the content of the editor is [saved](#editor), and restored on reload. `ID` must
be unique across all documents, e.g. a
...
```{rst:directive:option} reset: value (show | hide | auto)
...
```{js:attribute} preText
The text content of the `<pre>`{l=html} element contained in the directive. This
corresponds to the directive content, independently of an attached editor.
```

```{js:attribute} text
The text content of the editor if an editor is attached to the directive, or the
content of the `<pre>`{l=html} element contained in the directive
```

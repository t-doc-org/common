...
// A container for an RGBA color.
class Color {
    static parse(value) {
        let m = value.match(
            /^rgba?\((\d+),\s*(\d+),\s*(\d+)(?:,\s*(\d+\.?\d*))?\)$/);
        if (m) return new Color(...m.slice(1, 4).map(v => parseFloat(v)));
        m = value.match(
            /^#([0-9a-fA-F]{3}|[0-9a-fA-F]{4}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$/);
        if (m) {
            const v = str2Int(m[1], 16);
            switch (m[1].length) {
            case 3:
                return new Color((v >> 8) & 0xf, (v >> 4) & 0xf, v & 0xf);
            case 4:
                return new Color((v >> 12) & 0xf, (v >> 8) & 0xf,
                                 (v >> 4) & 0xf, (v & 0xf) / 0xf);
            case 6:
                return new Color((v >> 16) & 0xff, (v >> 8) & 0xff, v & 0xff);
            case 8:
                return new Color((v >> 24) & 0xff, (v >> 16) & 0xff,
                                 (v >> 8) & 0xff, (v & 0xff) / 0xff);
            }
        }
...
#${toRadix(this.r, 16, 2)}${toRadix(this.g, 16, 2)}${toRadix(this.b, 16, 2)}`;
        return `\
#${toRadix(this.r, 16, 2)}${toRadix(this.g, 16, 2)}${toRadix(this.b, 16, 2)}\
${toRadix(this.a * 0xff, 16, 2)}`;
    }
